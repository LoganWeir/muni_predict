...
class Labeling(object):
    ...
    def __init__(self, in_collection, out_collection):
        """
        Input:
    ...
    def get_gtfs_dir(self):
        """
        Uses a random date sample to get the local GTFS directory
        """
        # Get a sample from the collection to get a sample datetime object
        sample = self.in_coll.find_one()
        ...
        self.gtfs_directory = gtdys.get_gtfs_file(cln_date, gtfs_lookup_df)
    ...
    def get_all_starts(self, block):
        """
        For each row, check if it comes within 20 meters of the starting stop
    ...
    def cluster_starts(self, starts):
        ...
        for idx, item in enumerate(starts):
            ...
            cln_date = datetime.strptime(item['REPORT_TIME'], time_format)
            ...
            if len(start_time_clusters) == 0:
                ...
                for key, value in start_time_clusters.items():
                    ...
                    for time in value:
                        ...
                        row_time = time['REPORT_TIME']
                        row_dt = datetime.strptime(row_time, time_format)
